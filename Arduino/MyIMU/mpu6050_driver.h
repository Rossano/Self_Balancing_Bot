...
typedef union accel_gyro_union_t
{
  struct
  {
    uint8_t x_accel_h;
...
class mpu6050_driver
{
  public:
    ...
    mpu6050_driver();
    ~mpu6050_driver();
    int read_gyro_accel_values(uint8_t *);
    void calibrate_sensor();
    mpu_data get_mpu6050_data();
    void set_mpu6050_data(mpu_data);

  private:
    int mpu6050_i2c_read(int, uint8_t *, int);
    int mpu6050_i2c_write(int, const uint8_t *, int);
    int mpu6050_i2c_write_register(int, uint8_t);
    mpu_data calibrated_data;
    uint8_t buffer[20];
};
